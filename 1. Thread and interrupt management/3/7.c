...
void main()
{
    printf("Start thread scheduler test...\n");

    // create threads with different priorities
    for (int i = 0; i < NUM_THREADS; i++)
    {
        k_thread_create(&thread_data[i], thread_stacks[i], STACK_SIZE, thread, (void *)i, NULL, NULL, K_PRIO_COOP(i), 0, K_NO_WAIT);
    }
}
